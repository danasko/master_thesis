...
The model is heavily inspired by the implementation of the PointNet \cite{DBLP:journals/corr/QiSMG16}, which was built for the task of object classification and semantic segmentation. The architecture has been basically modified to fit the task of human pose estimation, making use of the auxiliary sub-network contributing to the global model loss.
...
\subsection{Initialization} % vahy init, optimizer Adam + lr + step decay, pocet bodov v pcl, rozmer poslednej vrstvy regression branch zavisi od poctu jointov v kostre, rozmer vystupu segmentacnej vetvy od poctu regionov tela, coz je ale v podstate rovnaky ako pocet jointov (viac o automatickej anotacii regionov tela v nasledujucej subsekcii o preprocessingu dat), 
