...
from utils import pointconv_util, tf_util
...
def feature_encoding_layer(xyz, feature, npoint, radius, sigma, K, mlp, is_training, bn_decay, weight_decay, scope,
...
def feature_decoding_layer(xyz1, xyz2, points1, points2, radius, sigma, K, mlp, is_training, bn_decay, weight_decay,
                           scope, bn=True, use_xyz=True):
    ''' Input:                                                                                                      
    ...
    with tf.variable_scope(scope) as sc:
        ...
        weight = weight_net_hidden(grouped_xyz, [32], scope='decode_weight_net', is_training=is_training,
                                   bn_decay=bn_decay, weight_decay=weight_decay)

        density_scale = nonlinear_transform(density_scale, [16, 1], scope='decode_density_net', is_training=is_training,
                                            bn_decay=bn_decay, weight_decay=weight_decay)
        ...
        if points1 is not None:
            new_points1 = tf.concat(axis=-1, values=[new_points, tf.expand_dims(points1,
                                                                                axis=2)])  # B,ndataset1,nchannel1+nchannel2
        else:
...
def feature_decoding_layer_depthwise(xyz1, xyz2, points1, points2, radius, sigma, K, mlp, is_training, bn_decay,
                                     weight_decay, scope, bn=True, use_xyz=True):
    ''' Input:                                      
...
if __name__ == '__main__':
    ...
    with tf.device('/gpu:0'):
        ...
        sub_pts, features = feature_encoding_layer(points_pl, features_pl, N, radius, sigma, K, [10, 20], is_training,
                                                   bn_decay=0.1, weight_decay=0.1, scope="FE")
        feature_decode = feature_decoding_layer(points_pl, sub_pts, features_pl, features, sigma, K, [10, 23],
                                                is_training, bn_decay=0.1, weight_decay=0.1, scope="FD")
