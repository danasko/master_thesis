...
def get_model_small_reg(point_cloud, is_training, cat_num, part_num, batch_size, num_point, weight_decay,
                        bn_decay=None):
    """ ConvNet baseline, input is BxNx3 gray image """
    end_points = {}

    net_transformed = tf.expand_dims(point_cloud, [2])
    ...
    net = tf_util.fully_connected(net, cat_num * 3, activation_fn=None, scope='cla/fc3')

    expand = tf.tile(out_max, [1, num_point, 1, 1])
    concat = tf.concat(axis=3, values=[expand, out4, out5])
...
def get_loss(l_pred, seg_pred, label, seg, weights):
    # per_instance_label_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=l_pred, labels=label)
    per_instance_label_loss = tf.reduce_mean(tf.squared_difference(l_pred, label))
    label_loss = tf.reduce_mean(per_instance_label_loss)  # tf.reduce_mean(per_instance_label_loss)
    ...
    total_loss = weights[1] * seg_loss + (weights[0] * label_loss)  # + mat_diff_loss * 1e-3
